...
.PHONY: docker-image
docker-image:
	docker build . -t logger:$(APP_VERSION) --no-cache \
...
push-image-gcp: ## push image to gcp
	@if [[ "$(docker images -q gcr.io/twinklymaha/logger:$(APP_VERSION)> /dev/null)" != "" ]]; then \
  @echo "Removing previous image $(APP_VERSION) from your machine..."; \
	docker rmi gcr.io/twinklymaha/logger:$(APP_VERSION);\
	fi
	docker build . -t gcr.io/twinklymaha/logger:$(APP_VERSION) --no-cache \
	--build-arg CLIENT_ID=$(CLIENT_ID) \
	--build-arg MQTT_HOST=$(MQTT_HOST) \
	--build-arg MQTT_PORT=$(MQTT_PORT) \
	--build-arg USER_NAME=$(USER_NAME) \
	...
	gcloud container images delete gcr.io/twinklymaha/logger:$(APP_VERSION) --force-delete-tags  || echo "no image to delete on the remote"
	docker push gcr.io/twinklymaha/logger:$(APP_VERSION)

push-and-serve-gcp: push-image-gcp deploy-existing-image

deploy-existing-image:
	gcloud compute instances create-with-container $(instance-name) \
		--container-image=gcr.io/twinklymaha/logger:$(DOCKER_IMAGE_TAG) \
		--machine-type=e2-micro \
		--subnet=default \
		--network-tier=PREMIUM \
		--metadata=google-logging-enabled=true \
		--tags=http-server,https-server \
		--labels=project=mqtt_logger \
...
update-instance:
	gcloud compute instances update-container $(instance-name) --container-image gcr.io/twinklymaha/logger:$(image-tag)
